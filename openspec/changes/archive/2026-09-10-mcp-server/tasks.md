...
- [x] 1.1 Extract the query functions from `startaste/dashboard/services.py`
...
- [x] 1.2 Add search queries — stars by text over full name and description with
...
- [x] 1.3 Add aggregation queries for top topics, top languages and top upvoted
...
- [x] 2.1 Add an explicit read-only opener in `startaste/db.py` using a SQLite
      URI with `mode=ro`; verify a test that a write on that connection is
      refused by the database itself
- [x] 2.2 Verify a test that opening read-only against a missing file raises a
      clear "run startaste sync first" error and creates no file
- [x] 2.3 Verify a test that a read on the read-only connection succeeds while
      another connection holds an open write transaction (WAL, from
      `sqlite-wal-mode`)

## 3. Token authentication

- [x] 3.1 Add a token authenticator reading records of `{name, hash, scopes}`
...
- [x] 3.2 Verify tests that a malformed record never authenticates and does not
      stop the valid records in the same file from working
- [x] 3.3 Add `startaste mcp-token` minting a token with at least 256 bits of
...
- [x] 3.4 Verify that starting the server with a missing or unreadable token
      file exits with a clear error rather than serving unauthenticated

## 4. The MCP server

- [x] 4.1 Add `startaste/mcp/` serving streamable HTTP with `/mcp` behind the
...
- [x] 4.2 Register the seven tools — `search_stars`, `search_upvotes`,
...
- [x] 4.3 Verify per-tool tests against a seeded database: results are bounded
...
- [x] 4.4 Add the `mcp` subcommand with `--host` (default `127.0.0.1`),
      `--port`, `--tokens-file`; verify the default bind is loopback and that
      arguments reach the server
- [x] 4.5 Add `mcp` and `uvicorn` to `requirements.txt` and the flake's package
      dependencies; verify `nix build .#default` succeeds and the built binary
      can start the MCP server

## 5. NixOS module

- [x] 5.1 Add `services.startaste.mcp.{enable,listenAddress,port,tokensFile}`
...
- [x] 5.2 Extend `nix/tests/module.nix`: with a token file provisioned in the
...
- [x] 6.1 Live check outside the VM: start the server against a real synced
...
- [x] 6.2 `pytest tests/ -v` passes, coverage does not regress, and
      `nix build .#default` plus `nix flake check` succeed
- [x] 6.3 Update CHANGELOG.md under Unreleased
- [x] 6.4 Confirm `mipnix` change `deploy-startaste-dapperehaan` group 5 needs
